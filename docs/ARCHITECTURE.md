...
- **A selection survives the sign-in round trip, and nothing is reserved during it.** The whole selection lives in the query string (`lib/booking-url.ts` builds both shapes, so the review page, the "Change" link and the booking action cannot drift on parameter names). A signed-out visitor on `/book` is offered **both** "Sign in" and "Create an account", each carrying that URL as `?next=`; `AuthForm` carries `next` across its own login↔signup link, and `signUp` puts it on the confirmation `emailRedirectTo` so it survives the inbox hop too. `createBookingAction` parses before it calls `requireUser`, so a session that lapsed while the review page sat open returns to _that_ selection rather than the home page.
  - This is deliberately **not** a stored "booking intent": no `Booking` row exists until an authenticated user confirms, so the hold and the `BookingSlot` unique constraint are never entered by a signed-out visitor and there is no half-made booking to reconcile or expire. The return URL is re-validated and re-priced by `quoteBooking` on the way back like any other — an hour taken while the user was signing up shows the normal "just taken" message.
  - The one hop `next` cannot cross is **password reset**: `resetPasswordForEmail` must be given a bare path with no query string (see "Password reset"), so a reset ends at `/login?reset=1`. Losing a selection behind a forgotten password is the accepted cost of that constraint.
...
- **Sender: `EMAIL_FROM`, one address for every message the app sends.** `sportsvenuebookings.com` is verified in Resend, so the deployed default is `noreply@sportsvenuebookings.com`. Per customer: add their domain in Resend → Domains, publish the DKIM/SPF (and, once trusted, DMARC) records it prints, then point `EMAIL_FROM` at an address on that domain. The value may be a bare address or a full `Name <address>` mailbox; a bare address is wrapped in `BRAND.shortName` by `fromAddress()`. `CONTACT_FROM_EMAIL` is the old name and is still read as a fallback. With neither set, mail falls back to Resend's shared test sender (`onboarding@resend.dev`), which needs no DNS but **only delivers to the address that owns the Resend account** — dev only.
- **Env vars** (all server-only, none `NEXT_PUBLIC_`): `RESEND_API_KEY`, `ADMIN_CONTACT_EMAIL`, `EMAIL_FROM`.
- **Password-reset email is not sent through Resend.** It is issued by Supabase Auth (`supabase.auth.resetPasswordForEmail`), so its From address comes from Supabase → Authentication → Emails/SMTP, **not** from `EMAIL_FROM`. To send it from the same domain, configure Resend as the custom SMTP provider in the Supabase dashboard.
